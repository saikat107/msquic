...
Goal
- Add or update tests so that the files changed in the current pull request reach the coverage target (ideally 100% line coverage) as measured by OpenCppCoverage (Cobertura XML).

Build, test, coverage inputs (Windows)
- build = ".\\scripts\\build.ps1"
- test = ".\\scripts\\test.ps1 -Filter BbrTest -CodeCoverage"
...
- A per-file coverage summary for those files from artifacts/coverage/msquiccoverage.xml.
