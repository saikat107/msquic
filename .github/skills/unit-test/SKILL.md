---
name: unit-test
description: This skill generates high quality unit tests for production code at scale for a focal function. Tests are idiomatic to existing suites, uncover product bugs and test new paths of the function that the existing test suite does not cover.
---

# Step 1
1. Locate the focal function {{focal}} and map it to the existing test suite in {{harness}}. You can use the query-repo skill to location the focal function. Query the semantic index db to learn about what the focal function does in context of it's callers and callees. It's very important that you consider the function summary and preconditions and postconditions of the focal function and its callees when generating tests, throughout this process.
...
If the summary for the focal function is not sufficient, query the semantic index for summaries of its callees to build context about what the focal function does in more detail.

# Step 3
3. For each test in the test suite, evaluate which path is covered for {{focal}} and then collect and save all of the paths that are still uncovered.
...
- **Conditions:** The complete and ordered sequence of logical conditions that must be met to follow this path. This must be the **FULL** path. Do not reference previous paths as a subpath, these paths must be understood as standalone paths, and all paths conditions must be expressed as top level conditions of the focal function, not its callees.
...
- Tests cover all edge cases, invalid input, memory safety issues, and other scenarios that are not covered by the current test suite. 
