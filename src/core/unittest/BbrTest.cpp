...
//
// Helper functions to set up BBR state for testing transitions FROM specific states
//
// Note: Bandwidth-driven state evolution (STARTUP→DRAIN→PROBE_BW) is not feasible in unit
// tests because it requires sophisticated packet metadata with HasLastAckedPacketInfo chains
// for delivery rate calculation. For tests that need to start from PROBE_BW or DRAIN to test
// a DIFFERENT mechanism (e.g., RTT expiration), we use the setup helpers below that manually
// configure state while ensuring all necessary invariants are maintained.
//

//
// Helper: Set up PROBE_BW state with valid invariants
// Use this only for testing transitions FROM PROBE_BW where the mechanism
// being tested (e.g., RTT expiration) doesn't depend on how we reached PROBE_BW.
//
static void SetupProbeBwState(
    QUIC_CONGESTION_CONTROL_BBR* Bbr,
    uint64_t TimeNow)
{
    // Set primary state
    Bbr->BbrState = BBR_STATE_PROBE_BW;
    Bbr->BtlbwFound = TRUE; // Must be true to have reached PROBE_BW
...
//
// Helper: Set up DRAIN state with valid invariants
// Use this only for testing transitions FROM DRAIN where the mechanism
// being tested (e.g., RTT expiration) doesn't depend on how we reached DRAIN.
//
static void SetupDrainState(
    QUIC_CONGESTION_CONTROL_BBR* Bbr,
    uint64_t TimeNow)
{
    // Set primary state
    Bbr->BbrState = BBR_STATE_DRAIN;
    Bbr->BtlbwFound = TRUE; // Must be true to have reached DRAIN
...
//
// ============================================================================
// CATEGORY: STATE MACHINE TRANSITION TESTS
// ============================================================================
//

//
// NOTE: T1 (STARTUP → DRAIN) and T2 (DRAIN → PROBE_BW) transitions are not tested here
// because they require precise bandwidth estimation control via packet metadata chains.
// These bandwidth-driven transitions are better suited for integration-level testing with
// network simulation. Bandwidth detection requires:
// - Packet metadata with HasLastAckedPacketInfo for delivery rate calculation
...
// How: Initialize in STARTUP, establish MinRtt, simulate time passage >10s,
//      then ACK with MinRttValid to trigger RttSampleExpired calculation.
//
TEST(BbrTest, StateTransition_StartupToProbeRtt_RttExpired)
{
    QUIC_CONNECTION Connection;
    QUIC_SETTINGS_INTERNAL Settings{};
    Settings.InitialWindowPackets = 10;
    ...
    AckEvent1.AdjustedAckTime = TimeNow;
    AckEvent1.MinRttValid = TRUE;
    AckEvent1.MinRtt = 50000; // 50ms
    AckEvent1.NumRetransmittableBytes = 1200;
    AckEvent1.NumTotalAckedRetransmittableBytes = 1200;
    AckEvent1.LargestAck = 1;
    AckEvent1.LargestSentPacketNumber = 1;
    ...
    ASSERT_FALSE(Bbr->RttSampleExpired);

    // Simulate time passage > 10 seconds (10,000,000 microseconds)
    TimeNow += 11000000;
    ...
    AckEvent2.TimeNow = TimeNow;
    AckEvent2.AdjustedAckTime = TimeNow;
    AckEvent2.MinRttValid = TRUE;
    AckEvent2.MinRtt = 60000; // Slightly higher RTT
    AckEvent2.NumRetransmittableBytes = 1200;
    AckEvent2.NumTotalAckedRetransmittableBytes = 2400;
    AckEvent2.LargestAck = 2;
    AckEvent2.LargestSentPacketNumber = 2;
...
// How: Set up PROBE_BW state, wait >10s, ACK with MinRttValid to trigger transition.
// Note: We use SetupProbeBwState() because bandwidth-driven state evolution is not
//       feasible in unit tests. This test focuses on RTT expiration mechanism.
//
TEST(BbrTest, StateTransition_ProbeBwToProbeRtt_RttExpired)
{
    ...
    // Set up PROBE_BW state with valid invariants
    SetupProbeBwState(Bbr, TimeNow);
...
// How: Enter PROBE_RTT, maintain low BytesInFlight, advance time 200ms+, complete
//      round trip, verify transition to PROBE_BW.
//
TEST(BbrTest, StateTransition_ProbeRttToProbeBw_ProbeComplete)
{
    QUIC_CONNECTION Connection;
    QUIC_SETTINGS_INTERNAL Settings{};
    Settings.InitialWindowPackets = 10;
    ...
    // Set to PROBE_RTT with BtlbwFound
    ...
    // ACK with low inflight to start probe timer
    QUIC_ACK_EVENT AckEvent1 = {};
    AckEvent1.TimeNow = TimeNow;
    AckEvent1.AdjustedAckTime = TimeNow;
    AckEvent1.MinRttValid = TRUE;
    AckEvent1.MinRtt = 50000;
    AckEvent1.NumRetransmittableBytes = 0;
    AckEvent1.NumTotalAckedRetransmittableBytes = 0;
    AckEvent1.LargestAck = 1;
    AckEvent1.LargestSentPacketNumber = 1;
    AckEvent1.IsLargestAckedPacketAppLimited = FALSE;
    AckEvent1.HasLoss = FALSE;
    ...
    // Advance time past 200ms probe duration
    TimeNow += 250000; // 250ms
    ...
    AckEvent2.AdjustedAckTime = TimeNow;
    AckEvent2.MinRttValid = TRUE;
    AckEvent2.MinRtt = 50000;
    AckEvent2.NumRetransmittableBytes = 0;
    AckEvent2.NumTotalAckedRetransmittableBytes = 0;
    AckEvent2.LargestAck = 2;
    AckEvent2.LargestSentPacketNumber = 2;
    AckEvent2.IsLargestAckedPacketAppLimited = FALSE;
    AckEvent2.HasLoss = FALSE;
...
// How: Enter PROBE_RTT without BtlbwFound, complete probe, verify transition to STARTUP.
//
TEST(BbrTest, StateTransition_ProbeRttToStartup_NoBottleneckFound)
{
    QUIC_CONNECTION Connection;
    QUIC_SETTINGS_INTERNAL Settings{};
    Settings.InitialWindowPackets = 10;
    ...
    // Set to PROBE_RTT WITHOUT BtlbwFound
    Bbr->BbrState = BBR_STATE_PROBE_RTT;
    Bbr->BtlbwFound = FALSE; // Key difference from previous test
    Bbr->MinRtt = 50000;
    Bbr->BytesInFlight = 0;
...
// How: Set up DRAIN state, simulate RTT expiration, ACK with MinRttValid to trigger transition.
// Note: We use SetupDrainState() because bandwidth-driven state evolution is not
//       feasible in unit tests. This test focuses on RTT expiration mechanism and transition priority.
//
TEST(BbrTest, StateTransition_DrainToProbeRtt_RttExpired)
{
    ...
    // Set up DRAIN state with valid invariants
    SetupDrainState(Bbr, TimeNow);
    ...
    // ACK with MinRttValid to trigger RTT expiration check
    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = TimeNow;
    AckEvent.AdjustedAckTime = TimeNow;
    AckEvent.MinRttValid = TRUE;
...
TEST(BbrTest, RecoveryWindowManagement)
{
    ...
    LossEvent.NumRetransmittableBytes = 2400; // 2 packets lost
    LossEvent.PersistentCongestion = FALSE;

    Connection.CongestionControl.QuicCongestionControlOnDataLost(&Connection.CongestionControl, &LossEvent);

    // Verify we're in recovery (not 0 = NOT_RECOVERY)
    ASSERT_NE(Bbr->RecoveryState, 0u);

    // ACK packets to progress through recovery
    uint64_t TimeNow = 1000000;
    for (int i = 0; i < 3; i++) {
        QUIC_ACK_EVENT AckEvent = {};
        AckEvent.TimeNow = TimeNow + (i * 10000);
        AckEvent.AdjustedAckTime = TimeNow + (i * 10000);
        AckEvent.MinRttValid = TRUE;
        AckEvent.MinRtt = 50000;
        AckEvent.NumRetransmittableBytes = 1200;
        AckEvent.NumTotalAckedRetransmittableBytes = 1200;
        AckEvent.LargestAck = 11 + i;
        AckEvent.LargestSentPacketNumber = 11 + i;
        AckEvent.IsLargestAckedPacketAppLimited = FALSE;
        AckEvent.HasLoss = FALSE;
        AckEvent.AckedPackets = NULL;

        // Send a new packet to keep traffic flowing
        Connection.CongestionControl.QuicCongestionControlOnDataSent(
            &Connection.CongestionControl, 1200);
    ...
    // Recovery window should be managed and non-zero
...
TEST(BbrTest, SendAllowanceFullyBlocked)
{
    ...
    // Set up scenario where BytesInFlight >= CongestionWindow
    uint32_t InitialCwnd = Bbr->CongestionWindow;

    // Send enough to fill the congestion window
    uint32_t BytesToSend = InitialCwnd;
    uint32_t PacketsToSend = (BytesToSend / 1200) + 1;
    ...
    // Verify BytesInFlight >= CongestionWindow
    ASSERT_GE(Bbr->BytesInFlight, Bbr->CongestionWindow);

    // Get send allowance - should be 0
    uint64_t TimeNow = 1000000;
    uint32_t SendAllowance = Connection.CongestionControl.QuicCongestionControlGetSendAllowance(
        &Connection.CongestionControl,
        0,       // TimeSinceLastSend (doesn't matter when fully blocked)
        FALSE);  // TimeSinceLastSendValid

    ASSERT_EQ(SendAllowance, 0u);
}
...
TEST(BbrTest, SendAllowanceBandwidthBased)
{
    ...
    // Manually set PROBE_RTT state (not STARTUP) to trigger bandwidth-based calculation
    Bbr->BbrState = BBR_STATE_PROBE_RTT;
    Bbr->MinRtt = 50000; // 50ms - above QUIC_SEND_PACING_INTERVAL
    Bbr->BtlbwFound = TRUE;

    // Set a reasonable bandwidth estimate using the WindowedMaxFilter
    QuicSlidingWindowExtremumUpdateMax(&Bbr->BandwidthFilter.WindowedMaxFilter, 100000, TimeNow);
    ...
    TimeNow += 10000; // Advance 10ms

    // Get send allowance with valid TimeSinceLastSend
    uint32_t SendAllowance = Connection.CongestionControl.QuicCongestionControlGetSendAllowance(
        &Connection.CongestionControl,
        TimeNow - LastSendTime,  // TimeSinceLastSend in microseconds
        TRUE);                    // TimeSinceLastSendValid

    // Should calculate based on bandwidth (not just window - BytesInFlight)
    // Allowance should be non-zero and calculated
    ASSERT_GT(SendAllowance, 0u);
}
...
TEST(BbrTest, SendQuantumHighPacingRate)
{
    QUIC_CONNECTION Connection;
    QUIC_SETTINGS_INTERNAL Settings{};
    Settings.InitialWindowPackets = 50; // Larger initial window
    Settings.PacingEnabled = TRUE;
    InitializeMockConnection(Connection, 1280);
    BbrCongestionControlInitialize(&Connection.CongestionControl, &Settings);
    ...
    // Set up high bandwidth to trigger high pacing rate path
    // kHighPacingRateThresholdBytesPerSecond = 1200000 (1.2 MB/s)
    // We need PacingRate >= 1200000 * BW_UNIT

    Bbr->MinRtt = 50000; // 50ms
    Bbr->BtlbwFound = TRUE;

    // Set very high bandwidth (10 Mbps = 10,000,000 bits/s = 1,250,000 bytes/s)
    QuicSlidingWindowExtremumUpdateMax(&Bbr->BandwidthFilter.WindowedMaxFilter, 1250000, TimeNow);

    // Update congestion window to reflect high bandwidth
    Bbr->CongestionWindow = 500000; // Large window

    // Send a packet first
    Connection.CongestionControl.QuicCongestionControlOnDataSent(
        &Connection.CongestionControl, 1200);

    // Trigger SetSendQuantum via OnDataAcknowledged
    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = TimeNow;
    AckEvent.AdjustedAckTime = TimeNow;
    AckEvent.MinRttValid = TRUE;
    ...
    // Send quantum should be calculated and set (non-zero)
    // The actual value depends on bandwidth and rate thresholds
...
TEST(BbrTest, TargetCwndWithAckAggregation)
{
    ...
    // Set up BBR with bottleneck found
    Bbr->BtlbwFound = TRUE;
    Bbr->MinRtt = 50000;

    // Populate bandwidth filter
    QuicSlidingWindowExtremumUpdateMax(&Bbr->BandwidthFilter.WindowedMaxFilter, 100000, TimeNow);

    // Populate MaxAckHeightFilter with aggregation data
    // This filter tracks excess ACKed bytes beyond expected
    QuicSlidingWindowExtremumUpdateMax(&Bbr->MaxAckHeightFilter, 5000, Bbr->RoundTripCounter);
    ...
    // ACK to trigger congestion window update with ACK aggregation
    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = TimeNow + 50000;
    AckEvent.AdjustedAckTime = TimeNow + 50000;
    AckEvent.MinRttValid = TRUE;
    ...
    // Congestion window should be calculated with ACK aggregation
    // With BtlbwFound and populated MaxAckHeightFilter, CWND should account for aggregation
    // This is implementation-dependent, but CWND should be influenced by the filter
    ASSERT_GT(Bbr->CongestionWindow, 0u);
}
...
TEST(BbrTest, AckTimingEdgeCase_FallbackToRawAckTime)
{
    ...
    // Send first packet
    Connection.CongestionControl.QuicCongestionControlOnDataSent(
        &Connection.CongestionControl, 1000);

    // Send a second packet (packet number will be 2)
    Connection.CongestionControl.QuicCongestionControlOnDataSent(
        &Connection.CongestionControl, 1000);

    // Simulate ACK: packet 2 has LastAckedPacketInfo with AdjustedAckTime > AckEvent.AdjustedAckTime
    // This will trigger the else branch (line 153-154)
    uint64_t BaseTime = 1000000;

    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = BaseTime + 10000;
    AckEvent.AdjustedAckTime = BaseTime + 5000; // Earlier than what we'll set in LastAckedPacketInfo
    AckEvent.MinRttValid = TRUE;
    AckEvent.MinRtt = 50000;
    ...
    // Verify BBR processed the ACK (basic sanity check - BytesInFlight reduced)
    ASSERT_LT(Bbr->BytesInFlight, 2000u);
}
...
TEST(BbrTest, InvalidDeliveryRates_BothMaxSkip)
{
    ...
    // ACK at the same time as sent - both rates will be invalid
    // This is an edge case where timing makes rates invalid
    uint64_t SameTime = 1000000;

    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = SameTime;
    AckEvent.AdjustedAckTime = SameTime;
    AckEvent.MinRttValid = FALSE; // No MinRtt update
    AckEvent.MinRtt = 0;
    ...
    // Should process without crashing, even with invalid rates
    Connection.CongestionControl.QuicCongestionControlOnDataAcknowledged(
        &Connection.CongestionControl, &AckEvent);

    // Bytes should be accounted for
    ASSERT_EQ(Bbr->BytesInFlight, 0u);
}
...
TEST(BbrTest, RecoveryGrowthState_WindowExpansion)
{
    ...
    LossEvent.LargestSentPacketNumber = 10;
    LossEvent.PersistentCongestion = FALSE;
    LossEvent.NumRetransmittableBytes = 3600; // 3 packets lost

    Connection.CongestionControl.QuicCongestionControlOnDataLost(
        &Connection.CongestionControl, &LossEvent);

    // BBR should now be in recovery
    ASSERT_NE(Bbr->RecoveryState, 0); // NOT_RECOVERY = 0

    uint32_t RecoveryWindowBefore = Bbr->RecoveryWindow;

    // ACK remaining packets - may trigger GROWTH state
    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = 1100000;
    AckEvent.AdjustedAckTime = 1100000;
    AckEvent.MinRttValid = TRUE;
    AckEvent.MinRtt = 50000;
    AckEvent.NumRetransmittableBytes = 7200; // 6 packets ACKed
    AckEvent.NumTotalAckedRetransmittableBytes = 7200;
    ...
    // Recovery window should be maintained or grown
    ASSERT_GE(Bbr->RecoveryWindow, RecoveryWindowBefore);
}
...
TEST(BbrTest, SendAllowanceBandwidthOnly_NonStartupPath)
{
    ...
    // Move to PROBE_RTT (non-STARTUP)
    Bbr->BbrState = BBR_STATE_PROBE_RTT;
    Bbr->MinRtt = 50000; // 50ms
    Bbr->MinRttTimestamp = 900000;
    Bbr->RttSampleExpired = TRUE;

    // Set bandwidth estimate
    QuicSlidingWindowExtremumUpdateMax(&Bbr->BandwidthFilter.WindowedMaxFilter, 1000000, 1000000);

    // Send a packet to establish LastSendTime
    Connection.CongestionControl.QuicCongestionControlOnDataSent(
        &Connection.CongestionControl, 1000);

    // Query send allowance with time elapsed (10ms = 10000 us)
    uint32_t SendAllowance = Connection.CongestionControl.QuicCongestionControlGetSendAllowance(
        &Connection.CongestionControl,
...
TEST(BbrTest, HighPacingRateSendQuantum_LargeBurstSize)
{
    ...
    // Set very high bandwidth to trigger high pacing rate path
    // kHighPacingRateThresholdBytesPerSecond = 1200000 (1.2 MB/s)
    // We need PacingRate >= 1200000 * BW_UNIT
    Bbr->MinRtt = 50000; // 50ms
    Bbr->BtlbwFound = TRUE;

    // Set very high bandwidth (10 Mbps = 1,250,000 bytes/s)
    QuicSlidingWindowExtremumUpdateMax(&Bbr->BandwidthFilter.WindowedMaxFilter, 1250000, 1000000);
    ...
    // ACK to trigger send quantum calculation
    QUIC_ACK_EVENT AckEvent = {};
    AckEvent.TimeNow = 1100000;
    AckEvent.AdjustedAckTime = 1100000;
    AckEvent.MinRttValid = TRUE;
