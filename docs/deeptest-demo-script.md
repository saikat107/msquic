...
> "Let's look at what the agent did. It first reads the PR file list to see what changed. Then — and this is where the semantic index pays off — it maps each changed source file to the relevant test harnesses. For MsQuic, that means files under `src/core/` map to specific GTest harnesses like `RangeTest.cpp` or `ConnectionTest.cpp`. DeepTest knows this because it has indexed the codebase structure."
>
